...
using shm_interfaces::msg::PodImage8m;
...
PodImage8m::UniquePtr V4l2CameraDevice::capture()
{
  auto buf = v4l2_buffer{};
  ...
  // Create image object
  auto img = std::make_unique<PodImage8m>();

  // Copy over buffer data
  auto const &buffer = buffers_[buf.index];
  size_t valid_size = buffer.length;
  if (valid_size > img->data.size())
  {
    throw std::runtime_error("Image size exceeds DATA_MAX_SIZE");
  }
  std::memcpy(img->data.data(), buffer.start, valid_size);
...
bool V4l2CameraDevice::setControlValue(uint32_t id, int32_t value)
{
  ...
  RCLCPP_INFO(
      rclcpp::get_logger(
          "v4l2_camera"),
      "Succesfully set value for control %s to %s", control->name.c_str(),
...
v4l2_camera::Control V4l2CameraDevice::queryControl(uint32_t id, bool silent)
{
  ...
  if (ioctl(fd_, VIDIOC_QUERYCTRL, &queryctrl) != 0)
  {
    if (!silent)
    {
      RCLCPP_ERROR_STREAM(
          rclcpp::get_logger("v4l2_camera"),
          "Failed querying control with ID: " << id << " - " << strerror(errno) << " (" << errno << ")");
    }
    return {};
  }
